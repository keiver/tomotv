 ...
 * withFFmpeg.js
 *
 * Expo config plugin that injects the TomoFFmpeg pod (our FFmpeg static
 * xcframeworks, see native/ios/TomoFFmpeg.podspec) into the generated
 * ios/Podfile.
 *
  ...
  return withDangerousMod(config, [
    ...
    async (config) => {
      ...
      if (!fs.existsSync(path.join(frameworksDir, "Libavformat.xcframework", "Info.plist"))) {
        throw new Error("[withFFmpeg] FFmpeg frameworks missing — run `npm run fetch:ffmpeg` before prebuild.");
      }
      ...
      if (!podfile.includes("TomoFFmpeg")) {
        ...
        if (updated === podfile) {
          throw new Error("[withFFmpeg] Could not find use_expo_modules! in Podfile to anchor the TomoFFmpeg pod.");
        }
        fs.writeFileSync(podfilePath, updated);
        console.log("[withFFmpeg] ✓ TomoFFmpeg pod added to Podfile");
      } else {
        console.log("[withFFmpeg] TomoFFmpeg pod already in Podfile");
      }
