...
## Note: Chapter Artwork Belongs in a Lazily Loaded AVMetadataItem, Never in the Player Item Build (September 2026)

react-native-video's `RCTVideoTVUtils.makeTimedMetadataGroup` fetched every chapter `uri` with a synchronous `Data(contentsOf:)` inside `preparePlayerItem`, which `setupPlayer` awaits, so a chaptered file with images could not start until the last image was down (#75; #71 had excluded the images for this reason and 488898c put them back small). The measured facts: the fetch runs on a detached task, so the UI never froze, only the start waited; and no library on the local Jellyfin has chapter image extraction on, so the case never reproduced here. The fix is Apple's own affordance, `AVMetadataItem(propertiesOf:valueLoadingHandler:)`, whose header says it exists for "navigation markers as consumed by AVPlayerViewController": the artwork item carries no value until AVKit's `AVNavigationMarkerImageProxy` calls `loadValuesAsynchronously` for it, read off the tvOS 26.5 simruntime AVKit binary with `strings`. Two Swift labels differ from the ObjC names, `respond(value:)` and `respond(error:)`; a `swiftc -typecheck` probe against the simulator SDK with a stubbed `RCTVideoUtils` caught both before any build. Rule: before re-setting a property on a live AVKit object and hoping it redraws, grep the SDK headers for a lazy or asynchronous variant of the value; AVKit usually has one and documents which consumer it serves.
