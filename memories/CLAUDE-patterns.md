...
- **Server transcoding:** only what is left: a codec the engine must re-encode above the pixel budget (VP9, software AV1 and the rest of `TRANSCODABLE_VIDEO_CODECS`; H.264, HEVC and hardware AV1 never meet it), a codec with no decoder in the build, no carriable audio track, or a metadata gap
